...
function ECGCanvas({ data, sev }) {
  ...
  return <canvas ref={ref} width={900} height={90}
...
function RhBar({ label, prob, active, accentColor }) {
  return <div style={{ marginBottom: 10 }}>
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
      <span style={{ color: active ? "#f0f4ff" : "#3a4a60", fontWeight: active ? 600 : 400 }}>{label}</span>
      <span style={{ color: active ? accentColor : "#2a3a50", fontVariantNumeric: "tabular-nums" }}>
        {(prob * 100).toFixed(1)}%
      </span>
    </div>
...
function Metric({ label, value, unit, sub, trend, color = "#4d9fff", warn = false }) {
  return <div style={{
    background: "#080e1a", border: `1px solid ${warn ? "#ffe06628" : "#111d2e"}`,
    borderRadius: 10, padding: "16px 18px", transition: "border-color .4s",
  }}>
    <div style={{ fontSize: 9, color: "#2a3d55", letterSpacing: ".14em", marginBottom: 6, fontWeight: 600 }}>{label}</div>
    <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 2 }}>
      <span style={{
        fontSize: 28, fontWeight: 700, color: warn ? "#ffe066" : color,
...
export default function App() {
  ...
  function toggleDemo() {
    if (demoRunning) {
      ...
      setError(null);
      fetchDemo();                              // immediate first call
      demoRef.current = setInterval(fetchDemo, 4000);
      setDemoRunning(true);
    }
  }
  ...
  async function runWfdb() {
    ...
    try {
      ...
      apply(await r.json());
      // auto-refresh WFDB result every 4s using same files
      clearInterval(demoRef.current);
      demoRef.current = setInterval(async () => {
        try {
          const fd2 = new FormData();
          fd2.append("dat_file", datFile);
          fd2.append("hea_file", heaFile);
          fd2.append("label",    ptbLabel);
          fd2.append("lead",     lead);
          fd2.append("device",   device);
          const r2 = await fetch(`${API_BASE}/triage/wfdb`, { method: "POST", body: fd2 });
  ...
  useEffect(() => {
    if (demoRunning) { clearInterval(demoRef.current); setDemoRunning(false); }
    if (streaming)   { sseRef.current?.close(); setStreaming(false); }
  }, [scenario, device]);
  ...
  const isLive = demoRunning || streaming;

  return (
    <div style={{ minHeight: "100vh", background: "#05080f", color: "#c0cfe8", fontFamily: "'JetBrains Mono','Fira Code','Courier New',monospace" }}>

      {/* noise overlay */}
      <div style={{
        position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, opacity: .4,
      ...
      <header style={{
        ...
        borderBottom: "1px solid #0d1828", background: "#05080fee",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 9, color: "#2a3d55", letterSpacing: ".1em" }}>
            <div style={{
              ...
              background: isLive ? "#0affb2" : "#111d2e",
              boxShadow: isLive ? "0 0 10px #0affb2,0 0 20px #0affb244" : "none",
              transition: "all .4s",
            }} />
            {isLive ? "LIVE" : "STANDBY"}
          </div>
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", padding: "28px 24px 60px", position: "relative", zIndex: 1 }}>
        ...
        {/* ECG strip — always animating */}
        <div style={{
          background: "#080e1a", border: `1px solid ${S.border}`,
          borderRadius: 12, padding: "16px 20px", marginBottom: 20,
          transition: "border-color .5s", boxShadow: `0 0 40px ${S.glow}18`,
        }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              {isLive && <span style={{ fontSize: 8, color: "#0affb2", letterSpacing: ".14em", animation: "pulse 1.5s ease-in-out infinite" }}>● REC</span>}
            </div>
          </div>
          <ECGCanvas data={ecgSig} sev={sev} />
        </div>
        ...
        {tab === "demo" && (
          <div style={{
            ...
            {/* Run Once */}
            <button onClick={runOnce} disabled={loading || isLive} style={{
              padding: "10px 18px", borderRadius: 8, fontSize: 10, fontWeight: 700,
              letterSpacing: ".1em", border: "1px solid #4d9fff40",
              background: loading || isLive ? "#080e1a" : "#4d9fff14", color: "#4d9fff",
              cursor: loading || isLive ? "not-allowed" : "pointer",
              fontFamily: "inherit", transition: "all .2s",
            }}>{loading ? "…" : "RUN ONCE"}</button>

            {/* Continuous demo */}
            <button onClick={toggleDemo} disabled={streaming} style={{
              padding: "10px 18px", borderRadius: 8, fontSize: 10, fontWeight: 700,
              letterSpacing: ".1em",
              border: `1px solid ${demoRunning ? "#ffe06640" : "#0affb230"}`,
              background: demoRunning ? "#ffe06614" : "#0affb214",
              color: demoRunning ? "#ffe066" : "#0affb2",
              cursor: streaming ? "not-allowed" : "pointer",
              fontFamily: "inherit", transition: "all .2s",
              textShadow: demoRunning ? "0 0 12px #ffe06666" : "0 0 12px #0affb266",
            }}>{demoRunning ? "■ PAUSE" : "▶ CONTINUOUS"}</button>

            {/* SSE stream */}
            <button onClick={toggleStream} disabled={demoRunning} style={{
              padding: "10px 18px", borderRadius: 8, fontSize: 10, fontWeight: 700,
              letterSpacing: ".1em",
              border: `1px solid ${streaming ? "#ff3c5f40" : "#4d9fff30"}`,
              background: streaming ? "#ff3c5f14" : "#4d9fff0a",
              color: streaming ? "#ff3c5f" : "#4d9fff",
              cursor: demoRunning ? "not-allowed" : "pointer",
              fontFamily: "inherit", transition: "all .2s",
            }}>{streaming ? "■ STOP" : "⚡ SSE"}</button>
          </div>
        )}
        ...
        {tab === "wfdb" && (
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr auto", gap: 12, alignItems: "end" }}>
              {[["DAT FILE", ".dat", datFile, setDatFile], ["HEA FILE", ".hea", heaFile, setHeaFile]].map(([lbl, ext, f, setF]) => (
                <div key={ext}>
        ...
        {error && (
          <div style={{
            ...
            <span>{error} — backend at <code style={{ opacity: .7 }}>{API_BASE}</code></span>
          </div>
        )}
        ...
        {result && (<>
          {result.requires_escalation && (
            <div style={{
              ...
              boxShadow: "0 0 30px #ff3c5f18", animation: "fadeIn .4s ease",
            }}>
              <div style={{
                width: 36, height: 36, borderRadius: 8, flexShrink: 0,
                background: "#ff3c5f18", border: "1px solid #ff3c5f40",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 16, color: "#ff3c5f",
              }}>■</div>
              <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 12 }}>
            <Metric label="HEART RATE" value={Math.round(result.heart_rate)} unit="bpm"
              trend={hist.hr} color={S.color} warn={result.heart_rate < 50 || result.heart_rate > 130} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ background: "#080e1a", border: "1px solid #111d2e", borderRadius: 10, padding: "18px 20px" }}>
              ...
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #0d1828", display: "flex", justifyContent: "space-between", fontSize: 10 }}>
                <span style={{ color: "#1a2d45" }}>RHYTHM: <span style={{ color: S.color, fontWeight: 700 }}>{result.rhythm_label}</span></span>
                <span style={{ color: "#1a2d45" }}>STRESS: <span style={{
                  color: result.stress_level === "High" ? "#ffe066" : result.stress_level === "Medium" ? "#ff9944" : "#0affb2",
                  fontWeight: 700,
                }}>{result.stress_level.toUpperCase()}</span></span>
              </div>
            </div>
            ...
            <div style={{ background: "#080e1a", border: "1px solid #111d2e", borderRadius: 10, padding: "18px 20px" }}>
              ...
              <div style={{
                ...
                padding: "14px 16px", borderRadius: 8, background: S.bg, border: `1px solid ${S.border}`,
              }}>
                <div style={{
                  width: 44, height: 44, borderRadius: 8, flexShrink: 0,
                  background: S.bg, border: `1px solid ${S.border}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 20, color: S.color, textShadow: `0 0 16px ${S.glow}`,
                }}>{S.icon}</div>
                <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {[
                  ...
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}>
                    ...
                    <span style={{ color: "#2a3d55", maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }}>{v}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {!result && !loading && (
          <div style={{ textAlign: "center", padding: "80px 0", color: "#111d2e", fontSize: 11, letterSpacing: ".18em" }}>
            SELECT A SCENARIO AND PRESS CONTINUOUS · OR UPLOAD A PTB-XL RECORD
          </div>
        )}
      </div>
